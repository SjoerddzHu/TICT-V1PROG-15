...
def connectionFcn():    # Connectie maken met de server
    global s
    global connection
    ...
    while connection == False :     # Als er geen connectie is probeert hij hem te maken
        try:
            ...
            host = socket.gethostbyname('192.168.42.1')   #IP van server
...
def RedswLed(ev=None):      # Als de rode knop ingedrukt word
    ...
    global RedLed_status
    global OraLed_status
    if GrnLed_status == 0 and RedLed_status == 1 and OraLed_status == 1:    # Als het groene lampje aan staat en het rode lampje uit
...
def sendPacket(value):    #Waneer er een packet verstuurd moet worden
    ...
    try:
        ...
        print('alarm')          # Op het moment dat er geen verbinding is runt hij opnieuw connectionFcn() om verbinding te zoeken anders gaat het alarm af
        connectionFcn()
        RedswLed()
